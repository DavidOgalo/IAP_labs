...
<body>
	...
	<form enctype="multipart/form-data" method="post" id="user_details" name="user_details" onsubmit="return validateForm()" action="<?=$_SERVER['PHP_SELF']?>" align="center">
		<h2>Register</h2>
		<div id="form-errors">
			<?php 
			session_start();
		...
		<div><input type="text" name="first_name" placeholder="First_Name"></div>
		<div><input type="text" name="last_name" placeholder="Last_Name"></div>
		<div><input type="text" name="city_name" placeholder="City_Name"></div>
		<div><input type="text" name="username" placeholder="Username"></div>
		<div><input type="password" name="password" placeholder="Password"></div>
		<div><label for="fileToUpload">Profile Photo:</label><input type="file" name="fileToUpload" id="fileToUpload"></div>
		<div><button type="submit" name="btn-save"><strong>SAVE</strong></button></div>
		<div><a href="login.php">Already registered? Login</a></div>
	</form>
	<table id="users-tab"  align="center">
		<tr><th>ID</th><th>USERNAME</th><th>FIRST NAME</th><th>LAST NAME</th><th>CITY</th><th>TIMESTAMP</th><th>ZONE OFFSET</th><th>PROFILE PHOTO</th></tr>
	<?php 
	...
	while ($user = $users->fetch_assoc()) {
		echo "<tr><td>".$user['id']."</td><td>".$user['username']."</td><td>".$user['first_name']."</td><td>".$user['last_name']."</td><td>".$user['user_city']."</td><td>".$user['timestamp']."</td><td>".$user['user_offset']."</td><td><img id='profile_pic' src='".$user['profile_photo']."'></td></tr>";
	}
